...
int main(int argc, char * argv[]) {
	...
	if(rank==0) { // This code is run by a single process
		...
		MPI_Recv(&intialValue, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
		...
		for (int i = 0; i < nProcesses - rank; i++) {
			...
			if (intialValue == -1) {
				intialValue = val;
			} else if (val > intialValue) {
					MPI_Send(&val, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
			} else {
					MPI_Send(&intialValue, 1, MPI_INT, rank + 1, 0, MPI_COMM_WORLD);
					int aux = intialValue;
					intialValue = val;
					val = aux;
			}
		...
		MPI_Send(&intialValue, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
